...
public class RequestUtils {
  ...
  /**
   * get的数据 获取HttpServletRequest到MAP
   * 
   * @param request
   * @return
   */
  ...
  public static String getSiteURL(HttpServletRequest request) {
    /*
     * StringBuffer url_buffer = request.getRequestURL(); String
     * name=request.getContextPath();
     * if(StringUtils.hasText(url_buffer)&&!("eb_sales_pool".equals(url_buffer))
     * &&StringUtils.hasText(name)){ StringBuffer url=new StringBuffer();
     * url.append(url_buffer.subSequence(0, url_buffer.indexOf(name) +
     * name.length())); return url.toString(); }else{ return
     * (String)SystemVariable.get("S_URL"); }
     */
  ...
  public static String rePageUrlParame(HttpServletRequest request, String reqPageName) {
    ...
    if (StringUtils.isEmpty(reqPageName)) {
      reqPageName = "p";
    }
  ...
  public static String getPageUrlParame(HttpServletRequest request, String reqPageName, String url) {
    ...
    if (!StringUtils.isEmpty(url)) {
      ...
      else if (!StringUtils.isEmpty(parame)) {
        return url + "?" + parame + "&" + reqPageName + "=";
      }
      else {
