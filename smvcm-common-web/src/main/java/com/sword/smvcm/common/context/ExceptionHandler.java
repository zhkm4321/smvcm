...
import com.sword.smvcm.exception.AjaxException;
import com.sword.smvcm.exception.SmvcmException;
import com.sword.smvcm.pojo.ErrorData;
import com.sword.smvcm.pojo.ErrorMessage;

/**
 * 创 建 人：gugu 创建日期：2015-04-20 修 改 人： 修改日 期： 描 述：异常拦截器，用异常的统一拦截，区分ajax请求与普通请求。
 */
public class ExceptionHandler implements HandlerExceptionResolver {
  ...
  public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object handler,
    ...
    // 控制台打印错误
    ex.printStackTrace();
    if (ex instanceof SmvcmException || ex instanceof IllegalArgumentException || ex instanceof RpcException) {
      String exMessage = ex.getMessage();
      if (ex instanceof RpcException) {
        exMessage = "远程调用失败，请联系管理员。";
      }
      if ("XMLHttpRequest".equalsIgnoreCase(request.getHeader("X-Requested-With")) || ex instanceof AjaxException) {// 不是ajax请求
